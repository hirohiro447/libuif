...
module handlers
  ...
  implicit none
    ...
    real(c_double) :: alpha 
    double precision :: xs(number),ys(number)
    integer :: i
  ...
  subroutine handlerMouseEvent(a, area, e)
    ...
    implicit none
      ...
      type(uiAreaMouseEvent) :: e
        ...
        if (i == (number + 1)) then   ! not in a point
           i = 0
        end if
...
! initialize
  ...
  yd = (/ 0.5, 0.8, 0.4, 0.22, 0.31, 0.54, 0.65, 0.94, 0.83, 0.12 , &
          0.34, 0.58, 0.99, 0.54, 0.31, 0.65, 0.77, 0.92, 0.41, 0.0 /)

! start procedure

  mainwin = uiNewWindow("libui Draw Sample"//c_null_char, 640, 480, 1)

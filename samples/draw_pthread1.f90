...
!
! gfortran -O2 -g -Wall -o ./draw_pthread0 draw_pthread0.f90 pcmodel.o -L. -lui `pkg-config gtk+-3.0 --libs` -ldl -lpthread
!
  ...
  subroutine count_up()
    ...
    use iso_c_binding
      ...
      integer :: comerr
        ...
        call pthread_create_opaque(threadptr,c_funloc(drawLabelup),nilp,comerr)

  end subroutine count_up
...
! select font size & position
      ...
      cx = dp%AreaWidth / 2.0d0 - fontSize * strlenw * 0.58d0 / 2.0d0
      if (cx < 0) then
        cx = 0.0d0
      end if
...
! start procedure
    ...
    drawText = "ボタンを押すとカウントアップします。"//nilc
    
! drawLabel update callback
    btn1 = uiNewButton("カウント開始"//c_null_char)
    call uiButtonOnClicked(btn1, c_funloc(count_up), c_null_ptr)
    call uiGridAppend(msggrid, btn1,0,1,1,1,1, uiAlignFill, 0, uiAlignFill)

!    call uiBoxSetPadded(vbox,1)
    ebtn = uiNewButton("終了"//c_null_char)
    call uiButtonOnClicked(ebtn, c_funloc(onClosing), c_null_ptr)
    call uiGridAppend(msggrid, ebtn,1,1,1,1,0, uiAlignFill, 0, uiAlignFill)

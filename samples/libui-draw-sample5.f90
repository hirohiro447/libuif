    ...
    use libuif
      ...
      type(uiDrawMatrix) :: m
        ...
        transformsp%DashPhase = 0
          ...
          path = uiDrawNewPath(uiDrawFillModeWinding)
          call uiDrawPathAddRectangle(path, 358.0d0, 80.5d0, 418.0d0 - 358.0d0, 140.5d0 - 80.5d0)
          call uiDrawPathEnd(path)
...
!	// save for when we do the translated one
          ...
          call uiDrawMatrixScale(m,438.0d0, 80.5d0,1.3d0, 1.3d0)
...
!	// for testing purposes, show what happens if we scale about (0, 0)
          call uiDrawMatrixSetIdentity(m)
          call uiDrawMatrixTranslate(m, -300.0d0, 50.0d0)
          call uiDrawTransform(p%Context, m)

          call uiDrawStroke(p%Context, path, original, originalsp)

          call uiDrawMatrixSetIdentity(m)
          call uiDrawMatrixScale(m,0.0d0, 0.0d0,1.3d0, 1.3d0)
          call uiDrawTransform(p%Context, m)

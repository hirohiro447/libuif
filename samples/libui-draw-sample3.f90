...
!// from https://msdn.microsoft.com/en-us/library/windows/desktop/dd756681%28v=vs.85%29.aspx
  subroutine drawD2DGeometryGroup(p) 
    use iso_c_binding
    use libuif_structs
    use libuif
      type(uiAreaDrawParams) :: p
      type(c_ptr) :: alternate;
      type(c_ptr) :: winding;
      type(uiDrawBrush) :: fill;
      type(uiDrawBrush) :: stroke;
      ...
      type(uiDrawMatrix) :: m
        ...
        call uiDrawPathNewFigureWithArc(alternate,105.0d0, 105.0d0,25.0d0,0.0d0, 2.0d0 * uiPi,0)
        call uiDrawPathNewFigureWithArc(alternate,105.0d0, 105.0d0,50.0d0,0.0d0, 2.0d0 * uiPi,0)
        call uiDrawPathNewFigureWithArc(alternate,105.0d0, 105.0d0,75.0d0,0.0d0, 2.0d0 * uiPi,0)
        call uiDrawPathNewFigureWithArc(alternate,105.0d0, 105.0d0,100.0d0,0.0d0, 2.0d0 * uiPi,0)
        call uiDrawPathEnd(alternate)

        winding = uiDrawNewPath(uiDrawFillModeWinding)
        call uiDrawPathNewFigureWithArc(winding,105.0d0, 105.0d0,25.0d0,0.0d0, 2.0d0 * uiPi,0)
        call uiDrawPathNewFigureWithArc(winding,105.0d0, 105.0d0,50.0d0,0.0d0, 2.0d0 * uiPi,0)
        call uiDrawPathNewFigureWithArc(winding,105.0d0, 105.0d0,75.0d0,0.0d0, 2.0d0 * uiPi,0)
        call uiDrawPathNewFigureWithArc(winding,105.0d0, 105.0d0,100.0d0,0.0d0, 2.0d0 * uiPi,0)
        call uiDrawPathEnd(winding)
...
!	// TODO the example doesn't provide these
        call d2dSolidBrush(fill, d2dForestGreen, 1.0d0)
        call d2dSolidBrush(stroke, d2dCornflowerBlue, 1.0d0)
        ...
        call uiDrawFill(p%Context, alternate, fill)
        call uiDrawStroke(p%Context, alternate, stroke, sp)
!	// TODO text

        call uiDrawMatrixSetIdentity(m)
        call uiDrawMatrixTranslate(m, 350.0d0, 100.0d0)
        call uiDrawTransform(p%Context, m)
        call uiDrawFill(p%Context, winding, fill)
        call uiDrawStroke(p%Context, winding, stroke, sp)
!//	// TODO text

        call uiDrawFreePath(winding)
        call uiDrawFreePath(alternate)

  end subroutine drawD2DGeometryGroup
...
!  drawD2DGeometryGroup
      call drawD2DGeometryGroup(p)
